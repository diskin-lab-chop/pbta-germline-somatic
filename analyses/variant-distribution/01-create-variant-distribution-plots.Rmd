...
cancer_gene_df <- read_tsv(file.path(input_dir, "cancerGeneList.tsv"))
...
plp <- plp %>%
  dplyr::mutate(gene_class = case_when(
    gene_symbol_vep %in% po_genes & gene_symbol_vep %in% ts_genes ~ "Both",
    gene_symbol_vep %in% po_genes ~ "Proto-oncogene",
    gene_symbol_vep %in% ts_genes ~ "Tumor suppressor gene",
    TRUE ~ "Unknown"
  )) %>%
  dplyr::mutate(gene_symbol_stylized = case_when(
    gene_class == "Both" ~ glue::glue("<b><i>{gene_symbol_vep}</i></b>"),
    gene_class == "Proto-oncogene" ~ glue::glue("<b>{gene_symbol_vep}</b>"),
    gene_class == "Tumor suppressor gene" ~ glue::glue("<i>{gene_symbol_vep}</i>"),
    TRUE ~ gene_symbol_vep
  ))
...
for (each in names(plplists)){
  ...
  n_per_gene <- plot_df %>%
    ...
    left_join(plot_df[,c("gene_symbol_vep", "gene_class", "gene_symbol_stylized")]) %>%
    distinct()
  ...
  dist_plot <- ggplot(data = n_per_gene, aes(x = reorder(gene_symbol_stylized, plot_order), y = hist_n, fill = plot_group)) +
...
for (each in names(plplists)){
  ...
  dist_plot <- ggplot(data = n_per_gene, aes(x = reorder(gene_symbol_stylized, plot_order), y = vc_n, fill = variant_classification_vep)) +
    geom_col(col = "black", size = 0.4) +
