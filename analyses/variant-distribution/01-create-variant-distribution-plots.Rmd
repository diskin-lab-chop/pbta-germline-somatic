...
```{r data wrangling}
plp <- read_tsv(file.path(data_dir, "pbta_germline_plp_calls.tsv"), show_col_types = FALSE) %>%
...
plp_som_not_cpg <- plp %>%
  filter(Hugo_Symbol %in% plp_somatic$Hugo_Symbol & !Hugo_Symbol %in% plp_cpg$Hugo_Symbol) %>%
  left_join(hist) %>%
  write_tsv(file.path(results_dir, "plp-variants-in-somatic-drivers-not-cpg.tsv"))
...
## make cpg plot excluding FGFR1 for R03 grant
if (each == "cpg") {
  
  dist_plot <- ggplot(data = n_per_gene[n_per_gene$Hugo_Symbol != "FGFR1",], aes(x = reorder(Hugo_Symbol, plot_order), y = hist_n, fill =    plot_group)) +
  geom_col(col = "black", size = 0.4) +
  scale_fill_manual(values = plot_group_palette, breaks = sort(names(plot_group_palette))) + 
  ylab("Number of patients with P/LP variants") +
  xlab(x_axis_title) +
  ...
  dev.set(dev.next())
  tiff(file.path(paste0(plot_dir, each, "-variant-distribution-by-histology_R03.tiff")), height = 1800, width = fig_width, res = 300)
  print(dist_plot)
  dev.off()
