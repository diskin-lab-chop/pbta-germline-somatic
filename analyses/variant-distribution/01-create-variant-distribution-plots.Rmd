...
plp <- read_tsv(file.path(root_dir, "analyses", "sample-distribution", "input", "merged_plp_res_popAF_filtered_plus_manual_11062023.tsv"), 
  ...
  select(Kids_First_Biospecimen_ID_normal, gene_symbol_vep, variant_classification_vep) %>%
...
vc_colors <- tibble(variant_classification_vep = unique(plp_cpg$variant_classification_vep),
                    vc_hex = RColorBrewer::brewer.pal(n = 12, name = 'Paired'))
...
```{r}
plp_full <- read_tsv(file.path(root_dir, "analyses", "two-hits", "input",
                               "merged_plp_full_res_popAF_filtered_plus_manual_11107023.tsv")) %>%
...
ggsave("/Users/corbettr/Documents/pbta-germline/manuscript/cpg-variant-allele-freq-hist.tiff",
       width = 4, height = 4.5)
