...
mb_hist <- hist %>%
  ...
  dplyr::mutate(cpg_plp = fct_relevel(cpg_plp,
                                      c("No P-LP", "CPG P-LP"))) %>%
  # correct three samples incorreclty labeled as having only biopsies; actually underwest partial resection
...
```{r}
shh_tp53_status_plp_enr <- plot_enr(mb_hist[!is.na(mb_hist$shh_subtype),], 
                       var1 = "tp53_status", 
                       var2 = "cpg_plp",
                       var1_names = unique(mb_hist$tp53_status[!is.na(mb_hist$shh_subtype)]), 
                       var2_names = c("No P-LP", "CPG P-LP"),
                       padjust = FALSE)

pdf(file.path(plot_dir, "mb_shh_plp_carrier_tp53_enr_heatmap.pdf"),
    width = 3.5, height = 3)
...
for (subtype in subtypes){
  ...
  mb_subtype_metastasis_plp_enr <- plot_enr(mb_hist[mb_hist$molecular_subtype == subtype,], 
                       var1 = "metastasis", 
                       var2 = "cpg_plp",
                       var1_names = sort(unique(mb_hist$metastasis[mb_hist$molecular_subtype == subtype])), 
                       var2_names = c("No P-LP", "CPG P-LP"),
                       padjust = FALSE)

  pdf(file.path(plot_dir, 
                glue::glue("mb_{subtype}_plp_carrier_metastasis_enr_heatmap.pdf")),
