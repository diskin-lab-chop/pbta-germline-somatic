...
for (group in groups){
  ...
  group_hist <- group_hist %>% 
    dplyr::mutate(mol_sub_group = case_when(
      !grepl("To be classified", mol_sub_group) ~ mol_sub_group,
      grepl("To be classified", mol_sub_group) ~ NA_character_
    )) %>%
    dplyr::mutate(extent_of_tumor_resection = case_when(
      !grepl("Unavailable", extent_of_tumor_resection) ~ extent_of_tumor_resection,
      TRUE ~ NA_character_
    ))
  ...
  add_model_os <- fit_save_model(group_hist,
                                  terms = ifelse(
                                     group %in% c("Low-grade glioma"), 
                                     "mol_sub_group+extent_of_tumor_resection+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                     ifelse(group %in% c("Other high-grade glioma"),
                                            "extent_of_tumor_resection+molecular_subtype+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                            ifelse(group == "DIPG or DMG",
                                                   "molecular_subtype+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                                   ifelse(group %in% c("Atypical Teratoid Rhabdoid Tumor", "Medulloblastoma"),
                                                          "extent_of_tumor_resection+mol_sub_group+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                                          ifelse(group %in% c("Mixed neuronal-glial tumor"),
                                                                 "extent_of_tumor_resection+mol_sub_group+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                                                 ifelse(group %in% c("Choroid plexus tumor", "Mesenchymal tumor",
                                                                                     "Meningioma", "Schwannoma"),
                                                                        "extent_of_tumor_resection+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                                                        "germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years"
                                                                        )))))),
             coxph_os_model_file,
             "multivariate",
             years_col = "OS_years",
  ...
  add_model_efs <- fit_save_model(group_hist,
                                  terms = ifelse(
                                     group %in% c("Low-grade glioma"), 
                                     "mol_sub_group+extent_of_tumor_resection+germline_sex_estimate+cpgPLP_status+age_at_diagnosis_years",
                                     ifelse(group %in% c("Other high-grade glioma"),
