...
for (j in 1:nrow(subtypes)) {
  ...
  if (subtypes$plot_group[j] %in% c("DIPG or DMG", "Other high-grade glioma")){
    
    subtype_hist <- hist %>%
      filter(grepl(subtypes$plot_group[j], plot_group) & molecular_subtype == subtypes$mol_sub_group[j])
  ...
  dplyr::mutate(extent_of_tumor_resection = fct_relevel(extent_of_tumor_resection,
                                                        c("Biopsy only", "Gross/Near total resection",
                                                          "Partial resection",
                                                          "Unavailable")))
