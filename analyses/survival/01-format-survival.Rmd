...
base_histologies_file <- file.path(input_dir, "survival-histologies-base-2023-09-20.tsv")

all_plp_file <- file.path(root_dir, "analyses", 
...
```{r}
# add OS data from base histologies file
hist_base <- read_tsv(base_histologies_file) %>%
  distinct(Kids_First_Participant_ID, .keep_all = T)

hist <- hist %>%
  dplyr::select(-OS_days, -OS_status, -EFS_days, -EFS_event_type) %>%
  left_join(hist_base[,c("Kids_First_Participant_ID", "OS_days", "OS_status", "EFS_days", "EFS_event_type")]) %>%
