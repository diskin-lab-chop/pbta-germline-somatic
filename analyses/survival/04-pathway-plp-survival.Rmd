...
km_mmr_output_pdf <- file.path(plots_dir,
                             glue::glue("MMR_km_survival_all_histologies.pdf"))

km_brca_output_pdf <- file.path(plots_dir,
                             glue::glue("BRCA_km_survival_all_histologies.pdf"))
```
...
```{r}
mmr_hist <- hgg_hist %>%
    filter(grepl("MMR PLP", germline_variant)) %>%
