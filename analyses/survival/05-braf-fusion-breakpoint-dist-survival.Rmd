...
fusions <- read_tsv(fusions_file) %>%
  ...
  dplyr::mutate(diagnosis = fct_relevel(diagnosis,
...
```{r}
int_braf_fusions_plp_efs_model <- fit_save_model(fusions[fusions$diagnosis == "pilocytic astrocytoma",],
...
```{r}
kap_pma_plp_efs <- survival_analysis(
  metadata  = fusions[fusions$diagnosis == "pilomyxoid astrocytoma",],
  ind_var = "cpg_plp",
  test = "kap.meier",
...
km_pma_plp_plot <- plotKM(model = kap_pma_plp_efs,
                  variable = "cpg_plp",
                  combined = F, 
                  title = "Pilomyxoid astrocytoma")
km_pma_plp_plot

ggsave(file.path(plot_dir, "km_EFS_pma_plp.pdf"),
       width = 8, height = 6, units = "in")



kap_pa_plp_efs <- survival_analysis(
  metadata  = fusions[fusions$diagnosis == "pilocytic astrocytoma",],
  ind_var = "cpg_plp",
  test = "kap.meier",
  metadata_sample_col = "Kids_First_Participant_ID", 
  days_col = "EFS_days",
...
km_pa_plp_plot <- plotKM(model = kap_pa_plp_efs,
                  variable = "cpg_plp",
                  combined = F, 
                  title = "Pilocytic astrocytoma")
km_pa_plp_plot

ggsave(file.path(plot_dir, "km_EFS_pa_plp.pdf"),
       width = 8, height = 6, units = "in")
```
...
```{r}
pa_fusions <- fusions %>%
  dplyr::filter(diagnosis == "pilocytic astrocytoma")
...
dev.off()
```
Assess extent of surgical resection by breakpoint group

```{r}
pa_plp_resection_enr <- plot_enr(pa_fusions[pa_fusions$extent_of_tumor_resection != "Not Reported",], 
                       var2 = "cpg_plp", 
                       var1 = "extent_of_tumor_resection",
                       var2_names = c("No P/LP", "CPG P/LP"), 
                       var1_names = unique(pa_fusions$extent_of_tumor_resection[pa_fusions$extent_of_tumor_resection != "Not Reported"]),
                       padjust = TRUE)

pdf(file.path(plot_dir, "breakpoint_group_resection_enr_heatmap.pdf"),
    width = 5.5, height = 3)
...
add_group_efs_model <- fit_save_model(fusions,
...
```{r}
add_group_plp_efs_model <- fit_save_model(fusions,
