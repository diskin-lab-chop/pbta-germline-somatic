...
mmr_bs <- plp %>%
      filter(gene_symbol_vep %in% mmr_genes & Kids_First_Biospecimen_ID_normal %in% hist$Kids_First_Biospecimen_ID_normal) %>%
...
hgg_hist <- hist %>%
  ...
  mutate(MMR_variant = case_when(
    Kids_First_Biospecimen_ID_normal %in% c(mmr_bs, mmr_bs_sv) ~ "MMR PLP",
    TRUE ~ "MMR WT"
  ))  %>%
...
hgg_hist <- hgg_hist %>%
  mutate(brca_variant = case_when(
    Kids_First_Biospecimen_ID_normal %in% c(brca_bs, brca_bs_sv) ~ "BRCA PLP",
    TRUE ~ "BRCA WT"
  ))  %>%
