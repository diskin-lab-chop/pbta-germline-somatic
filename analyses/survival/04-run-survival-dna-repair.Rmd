...
add_os_forest_plot <- plotForest(readRDS(file.path(results_dir, "coxph_hgg_OS_subtype_age_MMRvariant.RDS")))
...
readRDS(file.path(results_dir, "coxph_hgg_OS_subtype_age_MMRvariant.RDS"))
```


```{r coxph int EFS MMR plp}
int_efs_model <- fit_save_model(hgg_hist,
                             "age_at_diagnosis_years+mol_sub_group*MMR_variant",
                             file.path(results_dir, "coxph_int_hgg_EFS_subtype_age_MMRvariant.RDS"),
                             "multivariate",
                             years_col = "EFS_years",
                             status_col = "EFS_status")

int_efs_forest_plot <- plotForest(readRDS(file.path(results_dir, "coxph_int_hgg_EFS_subtype_age_MMRvariant.RDS")))

ggsave(file.path(plots_dir, "forest_hgg_int_EFS_subtype_age_mmrPLP.pdf"), 
       int_efs_forest_plot,
       width = 12, height = 3, units = "in",
       device = "pdf")

readRDS(file.path(results_dir, "coxph_int_hgg_EFS_subtype_age_MMRvariant.RDS"))
```
...
```{r}  
add_os_model <- fit_save_model(hgg_hist,
                             "mol_sub_group+age_at_diagnosis_years+brca_variant+MMR_variant",
