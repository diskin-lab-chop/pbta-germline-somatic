...
Re-plot TPM vs. PSI z-scores by splicing case and P/LP variant type (frameshift, stop gained, missense, and splice)
...
splice_events %>%
  dplyr::filter(splicing_case == "RI",
                variant_classification %in% c("frameshift variant", 
                                              ...
                                              "splice variant"),
                ((strand == "+" & min_dist < 1000) | (strand == "-" & max_dist > -1000))) %>%
  plot_tpm_vs_psi(., xlab = "Retained Intron PSI z-score")
...
splice_events %>%
  dplyr::filter(splicing_case == "SE",
                variant_classification %in% c("frameshift variant", 
                                              ...
                                              "splice variant"),
                ((strand == "+" & min_dist < 1000) | (strand == "-" & max_dist > -1000))) %>%
  plot_tpm_vs_psi(., xlab = "Skipped Exon PSI z-score")
...
splice_events %>%
  dplyr::filter(splicing_case %in% c("A5SS", "A3SS"),
                variant_classification %in% c("frameshift variant", 
                                              ...
                                              "splice variant"),
                ((strand == "+" & min_dist < 1000) | (strand == "-" & max_dist > -1000))) %>%
  plot_tpm_vs_psi(., xlab = "Alt. Splice Site PSI z-score")
