...
group_cts <- splice_psi_df %>%
  count(plot_case, plot_group) %>%
  dplyr::mutate(group_n = glue::glue("N = {n}"))

label_df <- splice_psi_df %>%
  left_join(group_cts) %>%
  dplyr::mutate(plot_case = case_when(
    splicing_case == "SE" ~ "Skipped Exon", 
    splicing_case == "RI" ~ "Retained Intron",
  ...
  dplyr::group_by(plot_group, plot_case) %>%
  dplyr::summarise(group_n = unique(group_n),
                   y_pos = min(sample_PSI_zscore) - 3,
                   .groups = "drop")
...
splice_psi_df %>%
    ...
    geom_jitter(aes(fill = variant_classification), shape = 21, 
                width = 0.2, size = 2.5, alpha = 0.75) +
    ...
    geom_text(data = label_df,
              aes(x = plot_group, y = y_pos, label = group_n),
              inherit.aes = FALSE, col = "grey20") +
