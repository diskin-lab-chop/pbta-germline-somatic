...
```{r}
predisposition_genes <- c("Gorlin syndrome" = "PTCH1|SUFU", "Li-Fraumeni syndrome" = "TP53", "NF-1" = "NF1", "NF-2" = "NF2", "Schwannomatosis" = "LZTR1", "Tuberous Sclerosis" = "TSC1|TSC2")
...
suppressWarnings(
  ...
  predisposition_review_ids <- predispositions_path_review %>%
    dplyr::filter(cancer_predispositions_path == predisposition) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  
  predisposition_list[[predisposition]] <- hist %>%
    # filter for patients with predisposition, either reported in hist OR confirmed through path report review
    filter(grepl(predisposition, cancer_predispositions) | Kids_First_Biospecimen_ID_normal %in% predisposition_review_ids) %>%
    ...
    dplyr::mutate(plp_variant = case_when(
      Kids_First_Biospecimen_ID_normal %in% plp$Kids_First_Biospecimen_ID_normal[plp$gene_symbol_vep %in% genes] ~ "Yes",
      TRUE ~ "No"
    ))
  }
)
...
predisposition_df %>%
  ...
  dplyr::mutate(predisposition = fct_relevel(predisposition,
                                      c("Schwannomatosis", "Gorlin syndrome", 
