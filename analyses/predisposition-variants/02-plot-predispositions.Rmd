...
syndromes <- read_tsv(predispositions_file) %>%
  ...
  dplyr::mutate(`Associated Syndrome` = case_when(
    ...
    `Associated Syndrome` == "DROSHA-associated tumor predisposition" ~ "DROSHA tumor predisposition",
...
```{r}
# Define label-making function
make_predisposition_label <- function(predisposition, genes_str) {
  formatted_genes <- gsub("\\|", ", ", genes_str)
  bquote(.(predisposition) ~ "(" * italic(.(formatted_genes)) * ")")
}

# Generate labels from predisposition_genes list
labels <- map2(names(predisposition_genes), predisposition_genes, make_predisposition_label)
names(labels) <- names(predisposition_genes)
...
# Define plot labels
make_label <- function(gene, syndrome) {
  bquote(italic(.(gene)) ~ "(" * .(syndrome) * ")")
}

#labels <- glue::glue("{syndrome_genes} ({syndromes$`Associated Syndrome`})")
labels <- map2(syndrome_genes, syndromes$`Associated Syndrome`, make_label)
