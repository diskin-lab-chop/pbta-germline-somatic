...
# intersect regions from all 4 bed files with 20bp PMBB padding
bedtools intersect -a $input_dir/xgen_plus_spikein.b38.20bp_padded.bed -b $input_dir/xgen-exome_hg38_liftover.merged.100bp_padded.bed | \
  ...
  sort -k1,1 -k2,2n | \
  bedtools merge > $results_dir/pmbb_pbta_wxs_intersect.20bp_padded.bed

# intersect regions from all 4 bed files with 100bp PMBB padding
bedtools intersect -a $input_dir/xgen_plus_spikein.b38.100bp_padded.bed -b $input_dir/xgen-exome_hg38_liftover.merged.100bp_padded.bed | \
  bedtools intersect -a - -b $input_dir/ashion_confidential_exome_v2_targets_hg38_paded100.bed | \
  bedtools intersect -a - -b $input_dir/S07604715_100bp_Padded.bed | \
  sort -k1,1 -k2,2n | \
