...
scores <- scores %>%
  ...
  group_by(Kids_First_Participant_ID, hallmark_name) %>%
      summarize(mean_gsea_score = median(gsea_score))
...
for (i in 1:length(repair_gene_source)){
  ...
  for (j in 1:length(sample_list)){
    ...
    gsva_res <- gsva_res %>%
      filter(!is.na(pvalue) & abs(score_diff) > 0.05) %>%
    ...
    gsva_res %>%
      ...
      ggplot(aes(x = score_diff, y = hallmark)) +
        ...
        xlim(-0.2, 0.45) +
        geom_text(aes(x = score_diff * 1.15, y = x_pos,
...
pathways <- c("HALLMARK_E2F_TARGETS", "HALLMARK_MYC_TARGETS_V2",
              "HALLMARK_G2M_CHECKPOINT", "HALLMARK_MYC_TARGETS_V1",
              "HALLMARK_MTORC1_SIGNALING", "HALLMARK_DNA_REPAIR")
  ...
  for (j in 1:length(pathways)){
    ...
    score_df <- scores %>%
      ...
      mutate(germline_variant = case_when(
        ...
        Kids_First_Participant_ID %in% otherRepair_ids ~ "Other DNA Repair",
        TRUE ~ "Non-repair"
      )) %>%
...
pathways <- c("KEGG_CELL_CYCLE", "KEGG_DNA_REPLICATION",
