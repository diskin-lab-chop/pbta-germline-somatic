...
ggsave(
  file.path(plot_dir, "gsva_diff_DNArepair_vs_other.pdf"), 
...
ggsave(
  file.path(plot_dir, "gsva_diff_MMR_vs_other.pdf"), 
  width = 30, height = 20, units = "cm")
```
...
ggsave(
  file.path(plot_dir, "gsva_diff_DNArepair_nonMMR_vs_other.pdf"), 
  width = 30, height = 20, units = "cm")
```
...
ggsave(
  file.path(plot_dir, "DNArepair_gsva_score_byGroup_violinPlot.pdf"), 
...
bs_df <- mmr_germline %>%
  ...
  left_join(hist[hist$sample_type == 'Tumor' & hist$experimental_strategy == 'RNA-Seq', c('Kids_First_Biospecimen_ID', 'Kids_First_Participant_ID')], by = 'Kids_First_Participant_ID')
```

```{r}
mmr_plp <- read_tsv(mmr_plp_file) %>%
  rename('Kids_First_Biospecimen_ID_normal' = 'Kids_First_Biospecimen_ID') %>%
  left_join(bs_df[,c('Kids_First_Biospecimen_ID_normal', 'Kids_First_Biospecimen_ID')], by = 'Kids_First_Biospecimen_ID_normal') %>%
  left_join(repair_scores[,c('Kids_First_Biospecimen_ID', 'gsea_score')], by = 'Kids_First_Biospecimen_ID') %>%
  write_tsv(file.path(results_dir, 'MMR_germline_variant_plp_with_DNArepair_gsea.tsv'))
``` 
