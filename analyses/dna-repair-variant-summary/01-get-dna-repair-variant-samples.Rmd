...
for (i in 1:length(repair_gene_source)){
  ...
  repair_germline <- germline %>%
    filter(gene_symbol_vep %in% repair_genes) %>%
  ...
  plot_df <- plot_df %>%
    ...
    hist_repair <- hist %>%
      dplyr::mutate(`All Repair` = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal ~ "Yes",
        TRUE ~ "No"
      )) %>%
      dplyr::mutate(BER = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal[repair_germline$BER == "Yes"] ~ "Yes",
        TRUE ~ "No"
      )) %>%
      dplyr::mutate(HR = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal[repair_germline$HR == "Yes"] ~ "Yes",
        TRUE ~ "No"
      )) %>%
      dplyr::mutate(MMR = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal[repair_germline$MMR == "Yes"] ~ "Yes",
        TRUE ~ "No"
      )) %>%
      dplyr::mutate(NER = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal[repair_germline$NER == "Yes"] ~ "Yes",
        TRUE ~ "No"
      )) %>%
      dplyr::mutate(NHEJ = case_when(
        Kids_First_Biospecimen_ID_normal %in% repair_germline$Kids_First_Biospecimen_ID_normal[repair_germline$NHEJ == "Yes"] ~ "Yes",
        TRUE ~ "No"
      )) %>%
