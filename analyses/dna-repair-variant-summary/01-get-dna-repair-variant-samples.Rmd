...
histologies_file <- file.path(root_dir, 'analyses', 'collapse-tumor-histologies',
                              'results',"germline-primary-plus-tumor-histologies-plot-groups.tsv")
...
Create data frame and add plot groups for plotting
...
plot_df <- repair_germline %>%
  ...
  # remove all repair
  filter(class != "DNA_repair")

# reorder descending total
plot_df$class <- factor(plot_df$class, levels = c('HR', 'BER', 'NER', 'MMR','NHEJ'))
plot_df$plot_group <- factor(plot_df$plot_group, levels = unique(plot_df$plot_group))
```

Plot DNA repair germline variant prevalence by plot group
