...
for (j in 1:length(repair_gene_source)){
  ...
  mmr_wt_germline <- hist %>%
    filter(!Kids_First_Biospecimen_ID_normal %in% mmr_germline) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  
  brca_wt_germline <- hist %>%
    filter(!Kids_First_Biospecimen_ID_normal %in% brca_germline) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  ...
  for (k in 1:length(plot_sigs)){
    ...
    sbs_df <- cosmicv3_sigs %>%
      filter(signature == plot_sigs[k] & Kids_First_Biospecimen_ID %in% c(brca_ids, brca_wt_ids)) %>%
    ...
    sbs_df <- cosmicv3_sigs %>%
      filter(signature == plot_sigs[k] & Kids_First_Biospecimen_ID %in% c(mmr_ids, mmr_wt_ids)) %>%
      mutate(germline_variant = case_when(
  ...
  mmr_df <- cosmicv3_sigs %>%
      filter(signature %in% mmr_sigs & Kids_First_Biospecimen_ID %in% c(mmr_ids, mmr_wt_ids)) %>%
  ...
  sbs3_df <- cosmicv3_sigs %>%
      dplyr::filter(signature == "SBS3" & Kids_First_Biospecimen_ID %in% c(mmr_ids, ctrl_ids)) %>%
      group_by(Kids_First_Biospecimen_ID) %>%
