...
for (i in 1:length(cosmic_files)){
  ...
  for (j in 1:length(repair_gene_source)){
    ...
    hr_germline <- dna_repair_plp %>%
      filter(grepl("BRCA|BARD1|BRIP|ATM|CHEK2|PALB2|RAD51", Hugo_Symbol) & !grepl("RAD51C", Hugo_Symbol)) %>%
      distinct(Kids_First_Biospecimen_ID) %>%
      pull(Kids_First_Biospecimen_ID)
...
for (i in 1:length(cosmic_files)){
  ...
  for (j in 1:length(repair_gene_source)){
    ...
    otherRepair_germline <- dna_repair_plp %>%
      filter(grepl("BRCA|BARD1|BRIP|ATM|CHEK2|PALB2", Hugo_Symbol)) %>%
      distinct(Kids_First_Biospecimen_ID) %>%
      pull(Kids_First_Biospecimen_ID)
    ...
    for (k in 1:length(plot_sigs)){
      ...
      sbs_df <- cosmicv3_sigs %>%
        ...
        mutate(germline_variant = case_when(
          ...
          Kids_First_Biospecimen_ID %in% otherRepair_ids ~ "BRCA/interacting",
          TRUE ~ "Non-repair"
        )) %>%
    ...
    mmr_df %>%
      ...
      collapsed_germline <- dna_repair_plp %>%
        filter(MMR == "Yes" & Hugo_Symbol %in% mmr_genes) %>%
        group_by(Kids_First_Biospecimen_ID) %>%
        summarise(Hugo_Symbol_MMRgermline = str_c(sort(unique(Hugo_Symbol)), collapse = "; ")) %>%
      ...
      mmr_df %>%
        ...
        select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID_normal, Kids_First_Biospecimen_ID_tumor,
               ...
               germline_variant, Hugo_Symbol_MMRgermline, Hugo_Symbol_somatic) %>%
