...
for (j in 1:length(repair_gene_source)){
  ...
  brca_germline <- dna_repair_plp %>%
    filter(grepl("BRCA|BARD1|BRIP|ATM|CHEK2|PALB2|RAD51|MRE11A", gene_symbol_vep)) %>%
    distinct(Kids_First_Biospecimen_ID_normal) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  
  otherRepair_germline <- dna_repair_plp %>%
    filter(!Kids_First_Biospecimen_ID_normal %in% c(mmr_germline, brca_germline)) %>%
    distinct(Kids_First_Biospecimen_ID_normal) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  
  ctrl_germline <- hist %>%
    filter(!Kids_First_Biospecimen_ID_normal %in% dna_repair_plp$Kids_First_Biospecimen_ID_normal) %>%
    pull(Kids_First_Biospecimen_ID_normal)
    ...
    cosmicv3_res %>%
      ...
      ggplot(aes(x = signature, y = exposure_diff, fill = association)) +
        ...
        labs(x = 'COSMIC v3.3 signature', y = 'exposure difference',
             fill = 'sig etiology', title = glue::glue("HGG, H3 WT, {plot_names[k]} P/LP vs. no P/LP")) +
...
for (j in 1:length(repair_gene_source)){
  ...
  otherRepair_germline <- hist %>%
    filter(Kids_First_Biospecimen_ID_normal %in% dna_repair_plp$Kids_First_Biospecimen_ID_normal & !Kids_First_Biospecimen_ID_normal %in% c(mmr_germline, brca_germline)) %>%
    pull(Kids_First_Biospecimen_ID_normal)
  ...
  tmb %>%
    dplyr::filter(Kids_First_Biospecimen_ID %in% hist$Kids_First_Biospecimen_ID_tumor) %>%
    mutate(germline_variant = case_when(
        Kids_First_Biospecimen_ID %in% mmr_ids ~ "MMR",
        Kids_First_Biospecimen_ID %in% brca_ids ~ "BRCA/interacting",
        Kids_First_Biospecimen_ID %in% otherRepair_ids ~ "Other repair",
    ...
    stat_compare_means(comparisons = comparisons_mmr, size = 3) +
  ...
  plp %>%
      ...
      write_tsv(file.path(results_dir, paste0("germline_variant_plp_with_MMRsigs.tsv")))
  ...
  dna_repair_plp %>%
      ...
      dplyr::rename('MMR_exposure' = 'sum') %>%
      left_join(tmb[,c('Kids_First_Biospecimen_ID', 'tmb')], by = "Kids_First_Biospecimen_ID") %>%
      write_tsv(file.path(results_dir, paste0("MMR_germline_variant_plp_with_MMRsigs.tsv")))
  ...
  collapsed_germline <- dna_repair_plp %>%
      dplyr::filter(gene_symbol_vep %in% c(mmr_genes, brca_genes)) %>%
  ...
  collapsed_onco <- maf %>%
      dplyr::filter(ONCOGENIC %in% c("Likely Oncogenic", "Oncogenic", "Resistance")) %>%
      dplyr::filter(Hugo_Symbol %in% c(mmr_genes, brca_genes)) %>%
  ...
  mmr_df %>%
      ...
      write_tsv(file.path(results_dir, paste0("PBTA_all_samples_with_MMRsigs.tsv")))
    
  # Generate summary table for all HGG subjects with SBS3 exposure  
  sbs3_df <- cosmicv3_sigs %>%
      dplyr::filter(signature == "SBS3" & Kids_First_Biospecimen_ID %in% c(mmr_ids, brca_ids, otherRepair_ids, ctrl_ids)) %>%
      group_by(Kids_First_Biospecimen_ID) %>%
    ...
    sbs3_df %>%
      ...
      dplyr::rename("SBS3_exposure" = exposure) %>%
      ...
      dplyr::select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID_normal, Kids_First_Biospecimen_ID_tumor,
             ...
             germline_variant, Hugo_Symbol_germline, Hugo_Symbol_somatic) %>%
      write_tsv(file.path(results_dir, paste0("PBTA_all_samples_with_SBS3sigs.tsv")))
