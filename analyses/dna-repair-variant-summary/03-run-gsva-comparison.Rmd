...
for (i in 1:length(repair_gene_source)){
  ...
  otherRepair_germline <- dna_repair_plp %>%
    filter(!Kids_First_Biospecimen_ID %in% c(mmr_germline, hr_germline) & Kids_First_Biospecimen_ID %in% hist_cbtn$Kids_First_Biospecimen_ID_normal) %>%
    distinct(Kids_First_Biospecimen_ID) %>%
    pull(Kids_First_Biospecimen_ID)
  ...
  for (j in 1:length(sample_list)){
    ...
    gsva_res %>%
      mutate(hallmark = factor(hallmark,
                               gsva_res$hallmark)) %>%
      dplyr::mutate(x_pos = as.numeric(rownames(gsva_res))) %>%
    ...
    # create barplot of score differences between groups
    kegg_res <- kegg_res %>%
      filter(!is.na(pvalue) & abs(score_diff) > 0.175) %>%
    ...
    kegg_res %>%
      ...
      dplyr::mutate(x_pos = as.numeric(rownames(kegg_res))) %>%

      ggplot(aes(x = score_diff, y = pathway,
                 label = ifelse(pvalue < 0.05, "*", ""))) +
      geom_bar(stat="identity", color = "black",
                 show.legend = FALSE) +
      theme_minimal() +
...
for (i in 1:length(repair_gene_source)){
  ...
  for (j in 1:length(hallmark_to_plot)){
    ...
    score_df <- hallmark_scores %>%
      filter(hallmark_name == hallmark_to_plot[j] & Kids_First_Participant_ID %in% c(mmr_ids, otherRepair_ids, ctrl_ids)) %>%
    ...
    # Define function to return standard deviations
    ...
    score_df %>%
      ...
      ggplot(aes(x = germline_variant, y = gsea_score, fill = germline_variant)) +
        geom_violin(binaxis = "y", stackdir = "center",
                    show.legend = FALSE) +
        geom_boxplot(width=0.1,
                    show.legend = FALSE) + 
        labs(x = 'Germline Variant Class', y = " GSVA score",
             title = hallmark_to_plot[j]) +
        theme_minimal() +
  ...
  for (j in 1:length(kegg_to_plot)){
    ...
    score_df <- kegg_scores %>%
      filter(kegg_name == kegg_to_plot[j] & Kids_First_Participant_ID %in% c(mmr_ids, otherRepair_ids, ctrl_ids)) %>%
