...
hist_cbtn <- read_tsv(cbtn_histologies_file) %>%
  dplyr::filter(grepl("DIPG or DMG|Other high-grade glioma", plot_group))
...
hallmark_to_plot <- c("HALLMARK_HEDGEHOG_SIGNALING", "HALLMARK_DNA_REPAIR", "HALLMARK_PROTEIN_SECRETION")
kegg_to_plot <- c("KEGG_GLIOMA", "KEGG_ERBB_SIGNALING_PATHWAY")
...
for (i in 1:length(repair_gene_source)){
  ...
  otherRepair_germline <- dna_repair_plp %>%
    dplyr::filter(!Kids_First_Biospecimen_ID_normal %in% c(mmr_germline, brca_germline)) %>%
    dplyr::pull(Kids_First_Biospecimen_ID_normal)
