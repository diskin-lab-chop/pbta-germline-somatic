...
```{r}
rna_ids <- hist_pbta %>% 
  filter(experimental_strategy == 'RNA-Seq' & sample_id %in% hist_cbtn$sample_id_tumor) %>%
  pull(Kids_First_Biospecimen_ID)

hallmark_scores <- hallmark_scores %>%
  filter(Kids_First_Biospecimen_ID %in% rna_ids) %>%
...
for (i in 1:length(repair_gene_source)){
  ...
  mmr_germline <- dna_repair_plp %>%
    filter(MMR == "Yes" & Kids_First_Biospecimen_ID %in% hist_cbtn$Kids_First_Biospecimen_ID_normal) %>%
    distinct(Kids_First_Biospecimen_ID) %>%
    pull(Kids_First_Biospecimen_ID)
  ...
  otherRepair_germline <- dna_repair_plp %>%
    filter(!Kids_First_Biospecimen_ID %in% c(mmr_germline, brca_germline) & Kids_First_Biospecimen_ID %in% hist_cbtn$Kids_First_Biospecimen_ID_normal) %>%
    distinct(Kids_First_Biospecimen_ID) %>%
    pull(Kids_First_Biospecimen_ID)
  ...
  for (j in 1:length(sample_list)){
    ...
    # create barplot of score differences between groups
    kegg_res <- kegg_res %>%
      filter(!is.na(pvalue) & abs(score_diff) > 0.175 & !grepl("GLYCOS", pathway))
...
for (i in 1:length(repair_gene_source)){
  ...
  brca_ids <- hist_pbta %>%
    filter(Kids_First_Participant_ID %in% Kids_First_Participant_ID[Kids_First_Biospecimen_ID %in% brca_germline]) %>%
    filter(sample_type == 'Tumor' & experimental_strategy == 'RNA-Seq') %>%
    pull(Kids_First_Participant_ID) %>%
    unique()
  ...
  for (j in 1:length(hallmark_to_plot)){
    ...
    score_df <- hallmark_scores %>%
      filter(hallmark_name == hallmark_to_plot[j] & Kids_First_Participant_ID %in% c(mmr_ids, brca_ids, otherRepair_ids, ctrl_ids)) %>%
  ...
  for (j in 1:length(kegg_to_plot)){
    ...
    score_df <- kegg_scores %>%
      filter(kegg_name == kegg_to_plot[j] & Kids_First_Participant_ID %in% c(mmr_ids, brca_ids, otherRepair_ids, ctrl_ids)) %>%
