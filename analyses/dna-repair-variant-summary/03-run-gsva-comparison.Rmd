...
for (i in 1:length(repair_gene_source)){
  ...
  for (j in 1:length(sample_list)){
    ...
    for (k in 1:length(hallmarks)){
      repair_scores <- scores[scores$hallmark_name == hallmarks[k] & scores$`Kids_First_Participant_ID` %in% repair_ids,]$gsea_score
      ctrl_scores <- scores[scores$hallmark_name == hallmarks[k] & scores$`Kids_First_Participant_ID` %in% ctrl_ids,]$gsea_score
    ...
    for (k in 1:length(pathways)){
      repair_scores <- kegg_scores[kegg_scores$kegg_name == pathways[k] & kegg_scores$`Kids_First_Participant_ID` %in% repair_ids,]$gsea_score
      ctrl_scores <- kegg_scores[kegg_scores$kegg_name == pathways[k] & kegg_scores$`Kids_First_Participant_ID` %in% ctrl_ids,]$gsea_score
