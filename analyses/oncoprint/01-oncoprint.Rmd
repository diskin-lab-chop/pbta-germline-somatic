...
```{r}
all_cases <- plp %>%
  dplyr::filter(!is.na(HGVSc_tumor) | CNV_status_somatic %in% c("gain", "loss") | (loh_OR > 1 & loh_pvalue < 0.05) | gene_LOH > 0.2) %>%
  dplyr::select(Kids_First_Biospecimen_ID_normal, Hugo_Symbol) %>%
  bind_rows(plp_sv %>% 
              dplyr::filter(!is.na(HGVSc_tumor) | CNV_status_somatic %in% c("gain", "loss") | gene_LOH > 0.2) %>%
              dplyr::select(Kids_First_Biospecimen_ID_normal, Hugo_Symbol)) %>%
...
alteration_mat <- alteration_df %>%
  dplyr::mutate(alterations = glue::glue("{Germline};{Somatic_Onco};{CNV};{LOH}")) %>%
...
```{r}
retain_samples <- apply(alteration_mat, 1, function(x) sum(grepl("GER", x) & (!grepl(";;;", x) | grepl("LOH", x))) > 0)
