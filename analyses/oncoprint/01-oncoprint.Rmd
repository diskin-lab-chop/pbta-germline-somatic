...
expr_zscore_file <- file.path(input_dir, "pbta-germline-838-gene-expr-zscores.tsv")

proteo_zscore_file <- file.path(input_dir, "cptac-hope-cpg-proteomics-zscores.tsv")
```

load hist and plp variant-somatic alterations merged files
```{r}
...
```{r}
all_cases <- plp %>%
  dplyr::filter(!is.na(HGVSc_tumor) | CNV_status_somatic %in% c("gain", "loss") | abs(loh_score) > 0.25 | gene_LOH > 0.25 | abs(expr_zscore) > 2 | abs(expr_zscore_proteo) >2) %>%
  dplyr::select(Kids_First_Biospecimen_ID_normal, Hugo_Symbol) %>%
  bind_rows(plp_sv %>% 
              dplyr::filter(!is.na(HGVSc_tumor) | CNV_status_somatic %in% c("gain", "loss") | gene_LOH > 0.25 | abs(expr_zscore) > 2 | abs(expr_zscore_proteo) >2) %>%
              dplyr::select(Kids_First_Biospecimen_ID_normal, Hugo_Symbol)) %>%
...
for (sample in unique(alteration_df$Kids_First_Biospecimen_ID_normal)){
  ...
  for (gene in unique(alteration_df$Hugo_Symbol)){
    ...
    if (sum(cnv$Kids_First_Biospecimen_ID_tumor %in% sample_tumor & cnv$Hugo_Symbol == gene & grepl("gain", cnv$CNV_status_somatic)) > 0){
      
      alteration_df$CNV[alteration_df$Kids_First_Biospecimen_ID_normal == sample & alteration_df$Hugo_Symbol == gene] <- "GAI"
      
    }
    
    if (sum(cnv$Kids_First_Biospecimen_ID_tumor %in% sample_tumor & cnv$Hugo_Symbol == gene & grepl("loss", cnv$CNV_status_somatic)) > 0){
    ...
    if (alteration_df$Germline[alteration_df$Kids_First_Biospecimen_ID_normal == sample & alteration_df$Hugo_Symbol == gene] == "GER"){
          ...
          if (sum(plp$Kids_First_Biospecimen_ID_tumor %in% sample_tumor & plp$Hugo_Symbol == gene & ((abs(plp$loh_score) > 0.25 & !is.na(plp$loh_score)) | (plp$gene_LOH > 0.25 & !is.na(plp$gene_LOH)))) > 0 | sum(plp_sv$Kids_First_Biospecimen_ID_tumor %in% sample_tumor & plp_sv$Hugo_Symbol == gene & plp_sv$gene_LOH > 0.25 & !is.na(plp_sv$gene_LOH)) > 0){
...
col = c("GAI" = "#c51b7d", "LOS" = "#4C47E3" , "MIS" = "#3CB371","FSI" ="#FFBBFF","NON" ="#6828E8","FSD" = "#56B4E9",
        "SPS" = "#F0E442")
```
...
annotation <- hist %>%
  ...
  dplyr::select(Kids_First_Biospecimen_ID_normal, germline_sex_estimate, predicted_ancestry, tumor_descriptor, plot_group, RNA_data_available, Proteomics_available, plot_group_hex) %>%
  column_to_rownames("Kids_First_Biospecimen_ID_normal") %>%
...
ht=oncoPrint(t(alteration_mat), get_type = function(x)strsplit(x, ";")[[1]],
             ...
             heatmap_legend_param = list(title ="Alterations",nrow = 13, title_position = "topleft",direction = "horizontal",
                                         at = c("GAI","LOS","MIS","NON","FSI","FSD", "SPS", "LOH", "XGAI", "XLOS", "PGAI", "PLOS", "GER"),
                                         labels = c("Copy gain", "Copy loss", "Missense","Nonsense","Frameshift Insertion","Frameshift Deletion",
                                                    "Splice Site", "Loss of Heterozygosity", "Expression Gain",
                                                    "Expression Loss", "Protein Gain", "Protein Loss", "Germline P/LP Variant")
             ))

tiff(file.path(plot_dir, "somatic-alteration-oncoprint.tiff"),
