...
hist <- hist %>%
    ...
    mutate(predicted_ancestry = case_when(
      is.na(predicted_ancestry) ~ "Unknown Ancestry",
...
clin <- hist %>%
  ...
  summarise(`Median age at diagnosis, years` = round(median(as.numeric(age_at_diagnosis_days)/365.25, na.rm = T), 2),
            `Median tumor mutation burden` = round(median(tmb, na.rm = T), 2)) %>%
  column_to_rownames("cpgPLP_status") %>%
  t() %>%
  as.data.frame() %>%
...
```{r}
terms = c("Predicted sex", "Race", "Ethnicity", "Genetic Ancestry Superpopulation",
          "Tumor Histology", "Median age at diagnosis, years",
          "Median tumor mutation burden")

sex_p <- fisher.test(table(hist$germline_sex_estimate[hist$germline_sex_estimate != "Unknown"], hist$cpgPLP_status[hist$germline_sex_estimate != "Unknown"]))$p.value
...
pvals <- data.frame(term = terms,
                    pvalue = c(sex_p, race_p, ethnicity_p, ancestry_p,
                               group_p, age_p, tmb_p)) %>%
  mutate(pvalue = ifelse(pvalue < 0.01,
...
```{r}
term_df <- tibble(Term = c("Predicted sex",
                           "Race",
                           "Ethnicity",
                           "Genetic Ancestry Superpopulation",
                           "Tumor Histology"))

merged_stat_df <- stats %>%
  bind_rows(term_df) %>%
  dplyr::mutate(Term = fct_relevel(Term,
                                   c("Number Cohort", "Predicted sex",
                                     sort(unique(hist$germline_sex_estimate)),
                                     "Race", sort(unique(hist$race)),
                                     "Ethnicity", sort(unique(hist$ethnicity)),
                                     "Genetic Ancestry Superpopulation",
                                     sort(unique(hist$predicted_ancestry)),
                                     "Tumor Histology",
                                     sort(unique(hist$plot_group))))) %>%
  dplyr::arrange(Term) %>%
  dplyr::mutate(`CPG P/LP` = case_when(
    is.na(`CPG P/LP`) ~ "",
    TRUE ~ `CPG P/LP`
  ),
  `No CPG P/LP` = case_when(
    is.na(`No CPG P/LP`) ~ "",
    TRUE ~ `No CPG P/LP`
  )) %>%
  left_join(pvals,
  ...
  dplyr::mutate(Term = case_when(
    !is.na(pvalue) | Term == "Number Cohort" ~ Term,
    is.na(pvalue) ~ glue::glue("     {Term}")
  )) %>%
  dplyr::mutate(pvalue = case_when(
    !is.na(pvalue) ~ pvalue,
    TRUE ~ "")) 

write.xlsx(merged_stat_df,
           file.path(results_dir, "demo-clin-stats-cohort.xlsx"),
           overwrite = TRUE,
           keepNA = TRUE,
           rowNames = FALSE)
```
...
terms <- terms[terms != "Tumor Histology"]
...
for (group in groups) {
  ...
  sex_p <- fisher.test(table(group_hist$germline_sex_estimate[group_hist$germline_sex_estimate != "Unknown"], group_hist$cpgPLP_status[group_hist$germline_sex_estimate != "Unknown"]))$p.value
  ...
  pvalues_hist <- data.frame(term = terms,
  ...
  hist_term_df <- tibble(Term = c("Predicted sex",
                           "Race",
                           "Ethnicity",
                           "Genetic Ancestry Superpopulation"))

  hist_stats_list[[group]] <- stats_hist %>%
    bind_rows(hist_term_df) %>%
    dplyr::mutate(Term = fct_relevel(Term,
                                     c("Number Cohort", "Predicted sex",
                                       sort(unique(group_hist$germline_sex_estimate)),
                                       "Race", sort(unique(group_hist$race)),
                                       "Ethnicity", sort(unique(group_hist$ethnicity)),
                                       "Genetic Ancestry Superpopulation",
                                       sort(unique(group_hist$predicted_ancestry))))) %>%
    dplyr::arrange(Term) %>%
    dplyr::mutate(`CPG P/LP` = case_when(
      is.na(`CPG P/LP`) ~ "",
      TRUE ~ `CPG P/LP`
...
subtype_enr_plot <- subtype_enr %>% 
  dplyr::mutate(subtype = str_replace_all(
      subtype,
      c(
        "\\bBRAF\\b" = "<i>BRAF</i>",
        "\\bTP53\\b" = "<i>TP53</i>"
