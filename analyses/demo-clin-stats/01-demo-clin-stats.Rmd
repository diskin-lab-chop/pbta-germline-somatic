...
hist <- hist %>%
    dplyr::mutate(race = case_when(race %in% unk_race ~ "Other/Unknown Race",
                                   TRUE~ race),
                  ethnicity = case_when(ethnicity %in% unk_ethnicity ~ "Other/Unknown Ethnicity",
                                        TRUE ~ ethnicity),
                  predicted_ancestry = case_when(is.na(predicted_ancestry) ~ "Unknown/Mixed Ancestry",
                                                 TRUE ~ predicted_ancestry),
                  sex = case_when(germline_sex_estimate != "Unknown" ~ germline_sex_estimate,
                                  TRUE ~ reported_gender),
                  plot_group = case_when(plot_group == "Atypical Teratoid Rhabdoid Tumor" ~ "ATRT",
    TRUE ~ plot_group)
    )
```
...
sex <- summarize_count(hist, "cpgPLP_status", "sex")
...
sex_p <- fisher.test(table(hist$sex, hist$cpgPLP_status))$p.value
...
for (group in groups) {
  ...
  sex <- summarize_count(group_hist, "cpgPLP_status", "sex")
...
for (group in groups) {
  ...
  sex_p <- fisher.test(table(group_hist$sex, group_hist$cpgPLP_status))$p.value
