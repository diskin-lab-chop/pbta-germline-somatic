...
subtypes <- hist %>%
  count(mol_sub_group) %>%
  dplyr::filter(n >= 8, !is.na(mol_sub_group)) %>%
  pull(mol_sub_group)
...
subtype_cts <- hist %>%
  ...
  select(subtype, cpgPLP_status) %>%
  group_by(subtype, cpgPLP_status) %>%
  count() %>%
  ungroup() %>%
  ...
  select(subtype, cpgPLP, no_cpgPLP, OR, P_Value,
         CI_lower, CI_upper) %>%
  ungroup() %>%
...
subtype_enr <- subtype_cts %>% 
  dplyr::filter(subtype != "Other") %>%
  ...
  geom_text(x = log10(subtype_cts$CI_upper[subtype_cts$subtype != "Other"])+0.25, hjust = 0, vjust = .5, size = 4, fontface = 2) +
