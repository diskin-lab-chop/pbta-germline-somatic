...
all_plp_file <- file.path(input_dir, 
                      "pbta-germline-autogvp-full-may2024-adjacent-indels-rm-vaf10.tsv")

plp_sv_file <- file.path(root_dir, "analyses",
                         "germline-sv", "input", "pbta_germline_svs.tsv")
...
cpg_bs <- all_plp %>%
  ...
  dplyr::mutate(vaf = AD_alt/(AD_ref + AD_alt)) %>%
  filter(vaf >=0.2) %>%
