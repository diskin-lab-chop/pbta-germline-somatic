...
hist_tumor <- hist %>%
  dplyr::filter(sample_type == "Tumor" & experimental_strategy == "RNA-Seq" & Kids_First_Participant_ID %in% hist_cohort$Kids_First_Participant_ID) %>%
...
subset_expr <- expr[,intersect(hist_tumor$Kids_First_Biospecimen_ID_rna, colnames(expr))]
...
hist_cohort <- hist_cohort %>%
  left_join(rna_specimens[,c("Kids_First_Participant_ID", "Kids_First_Biospecimen_ID")]) %>%
  dplyr::rename(Kids_First_Biospecimen_ID_rna = Kids_First_Biospecimen_ID)

z_scored_expression <- z_scored_expression %>%
  # Add BS normal ids
  left_join(hist_cohort[,c("Kids_First_Biospecimen_ID_normal", "Kids_First_Biospecimen_ID_rna")]) %>%
...
z_scored_expression <- z_scored_expression %>%
  ...
  left_join(hist_cohort[,c("Kids_First_Biospecimen_ID_rna", "plot_group", "plot_group_hex")]) %>%
  ...
  dplyr::filter(group_gene %in% plot_group_gene) %>%
   ...
   ggplot(aes(x = is_plp, y = expr_zscore)) + 
    ...
    facet_wrap(~group_gene, nrow = 2,
               labeller = labeller(group_gene = label_wrap_gen(.5))) + 
    theme_Publication()
