...
Split hist file into normal and tumor RNA IDs, to be used to append matched tumor RNA samples to normal samples in `germline`
...
z_scored_expression %>%
  ...
  geom_jitter(aes(color = plp_group), width = 0.3, size = 2.5, alpha = 0.85) +
  geom_boxplot(alpha = 0.25) +
  stat_compare_means(comparisons = list(c("Yes", "No")),
                       method.args = list(alternative = "two.sided")) + 
  labs(x = "Germline P/LP variant", y = "expression z-score", color = "Histology") +
  ylim(c(-5,8)) +
  scale_color_manual(values = plot_group_palette, breaks = sort(names(plot_group_palette)), na.translate = F) +
...
z_scored_expression %>%
  dplyr::filter(group_gene %in% plot_group_gene & group_gene != "NF1:Neurofibroma plexiform") %>%
   ...
   ggplot(aes(x = is_plp, y = expr_zscore)) + 
    ...
    stat_compare_means(comparisons = list(c("Yes", "No")),
                       method.args = list(alternative = "two.sided")) + 
    labs(x = "Germline P/LP variant", y = "expression z-score") +
    ylim(c(-3,4.5)) +
    scale_color_manual(values = plot_group_palette, breaks = sort(names(plot_group_palette)), na.translate = F) +
  ...
  if (cpg %in% c("NF1", "TP53")) {
    ...
    z_scored_expression %>%
      ...
      stat_compare_means(comparisons = list(c("+\n-", "-\n-"),
                                            ...
                                            c("+\n-", "+\n+")),
                       method.args = list(alternative = "two.sided")) + 
      labs(title = cpg, x = NULL, y = "expression z-score", color = "Histology") +
      scale_color_manual(values = plot_group_palette, breaks = sort(names(plot_group_palette)), na.translate = F) +
      theme_Publication() +
