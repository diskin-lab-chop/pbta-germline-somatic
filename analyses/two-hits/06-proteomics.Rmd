...
hist_v13_file <- file.path(input_dir, "histologies-base.tsv")
...
hist_v13 <- read_tsv(hist_v13_file)

hist_v13_proteo <- hist_v13 %>%
...
hist <- read_tsv(hist_file) %>%
  left_join(hist_v13_proteo[,c("Kids_First_Biospecimen_ID_proteo",
                           "sub_cohort", "sample_id_tumor", "match_id")],
            by = "sample_id_tumor") %>%
  left_join(hist_v13_phospho[,c("Kids_First_Biospecimen_ID_phospho",
                           "match_id")],
            by = "match_id") %>%
...
cptac_proteo <- cptac_proteo %>%
  dplyr::select(-Proteins, -MaxPepProb, -ReferenceIntensity, -NumberPSM) %>%
  dplyr::rename(GeneSymbol = ApprovedGeneSymbol)

# change column names to sample IDs
colnames(cptac_proteo)[2:ncol(cptac_proteo)] <- substring(colnames(cptac_proteo)[2:ncol(cptac_proteo)], 3, nchar(colnames(cptac_proteo)[2:ncol(cptac_proteo)]))

# Identify duplicated sample IDs
samples <- str_replace(names(cptac_proteo)[-1], "\\.1|\\.2|\\.3|\\.4", "")
duplicated_samples <- samples[duplicated(samples)]

# calculate average abundances for sample IDs
for (sample in duplicated_samples) {
  cptac_proteo[[sample]] <- rowMeans(cptac_proteo[, grepl(sample, names(cptac_proteo))], na.rm = TRUE)
}

# Remove duplicated columns
cptac_proteo <- cptac_proteo %>%
  dplyr::select(-ends_with(c(".1", ".2", ".3", ".4")))
...
hope_proteo <-  hope_proteo %>%
  dplyr::select(-Proteins, -MaxPepProb, -ReferenceIntensity, -NumberPSM) %>%
  dplyr::rename(GeneSymbol = ApprovedGeneSymbol)

# Convert column names to sample IDs
colnames(hope_proteo)[2:ncol(hope_proteo)] <- substring(colnames(hope_proteo)[2:ncol(hope_proteo)], 3, nchar(colnames(hope_proteo)[2:ncol(hope_proteo)]))
colnames(hope_proteo) <- str_replace(colnames(hope_proteo), "UP", "")

# Identify duplicated sample IDs
samples <- str_replace(names(hope_proteo)[-1], "\\.1|\\.2", "")
duplicated_samples <- samples[duplicated(samples)]

# calculate averages
for (sample in duplicated_samples) {
  hope_proteo[[sample]] <- rowMeans(hope_proteo[, grepl(sample, names(hope_proteo))], na.rm = TRUE)
}
...
Merge hope and cptac proteomics data. In cases of duplicated samples, calculate average abundance. 
...
phospho_mat <- hope_phospho %>%
  dplyr::select(-GeneSymbol, -Peptide_seqeunce) %>%
...
phospho_mat <- phospho_mat[,!is.na(names(phospho_mat))]
```
...
z_scored_phospho <- z_scored_phospho %>%
  ...
  gather(key = "sample_id_tumor", value = "expr_zscore_phospho", -Site) %>%
  left_join(hope_phospho[,c("Site", "GeneSymbol")]) %>%
  left_join(hist[,c("Kids_First_Biospecimen_ID_normal", "sample_id_tumor")]) %>%
  dplyr::select(Kids_First_Biospecimen_ID_normal, sample_id_tumor,
                GeneSymbol, Site, expr_zscore_phospho) %>%
  distinct(sample_id_tumor, Site, .keep_all = T) %>%
  dplyr::filter(GeneSymbol %in% cpgs) %>%
...
plp_cpg_proteo <- plp_cpg_proteo %>%
  ...
  distinct(Kids_First_Biospecimen_ID_normal, GeneSymbol, Site, .keep_all = T) %>%
  dplyr::select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID_normal,
                ...
                expr_zscore_proteo, Site, expr_zscore_phospho)
