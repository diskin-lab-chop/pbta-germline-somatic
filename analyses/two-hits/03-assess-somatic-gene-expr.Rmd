...
z_scored_expression <- z_scored_expression %>%
  dplyr::mutate(
    subgroup_clean = subgroup %>%
      gsub("Other high-grade glioma", "HGG", .) %>%
      gsub("Low-grade glioma", "LGG", .) %>%
      gsub("MB, SHH", "MB-SHH", .) %>%
      gsub(",", "", .) %>%
      gsub(" ", "~", .),
      group_gene = glue::glue('**<i>{Hugo_Symbol}</i>: {subgroup_clean}**')
    )
