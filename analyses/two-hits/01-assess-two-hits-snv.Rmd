...
v14_hist_file <- file.path(root_dir, "analyses",
                           "collapse-tumor-histologies",
                           "input", "histologies.tsv")

plp_sv <- file.path(root_dir, "analyses", "germline-sv",
...
v14_hist <- read_tsv(v14_hist_file, guess_max = 3000) %>%
...
maf <- data.table::fread(file.path(root_dir, params$maf_in), data.table = F) %>%
  filter(Tumor_Sample_Barcode %in% v14_hist$Kids_First_Biospecimen_ID)
