...
lof_hits_subset <- lof_hits %>%
  ...
  dplyr::select(Matched_Norm_Sample_Barcode, Tumor_Sample_Barcode, Hugo_Symbol, Transcript_ID, HGVSc, HGVSp, VAF, SIFT, PolyPhen, Variant_Classification, HotSpotAllele, ONCOGENIC, MUTATION_EFFECT, VARIANT_IN_ONCOKB)
...
collapsed <- germline %>%
  dplyr::filter(Hugo_Symbol %in% cpg) %>%
...
germ_maf <- germline_somatic_gene_matches %>%
  ...
  bind_rows(germline %>%
                dplyr::filter((Hugo_Symbol == "LZTR1" & Matched_Norm_Sample_Barcode %in% c("BS_ETAATG3J", "BS_FPB0JGKX")) |
                                 (Hugo_Symbol == "PTCH1" & Matched_Norm_Sample_Barcode == "BS_MN2FX68Z") |
                                (Hugo_Symbol == "GBA" & Matched_Norm_Sample_Barcode %in% c("BS_MN2FX68Z", "BS_HVCBQ58H"))) %>%
                dplyr::select(any_of(names(germline_somatic_gene_matches)))) %>%
    left_join(integrated_hist %>%
                dplyr::rename(Tumor_Sample_Barcode = Kids_First_Biospecimen_ID_tumor,
                              Matched_Norm_Sample_Barcode = Kids_First_Biospecimen_ID_normal) %>%
                dplyr::select(Tumor_Sample_Barcode, Matched_Norm_Sample_Barcode),
              by = "Matched_Norm_Sample_Barcode") %>%
  ...
  dplyr::mutate(Variant_Classification = case_when(
    ...
    grepl("splice", Variant_Classification) ~ "Splice_Region",
    grepl("synonymous", Variant_Classification) ~ "Silent",
  ...
  dplyr::mutate(HGVSp = case_when(
    Matched_Norm_Sample_Barcode == "BS_MN2FX68Z" ~ "p.X68_splice",
    Matched_Norm_Sample_Barcode %in% c("BS_MN2FX68Z", "BS_HVCBQ58H") ~ "p.X39_splice",
    Matched_Norm_Sample_Barcode == "BS_9A5CF1QE" ~ "p.X326_splice",
    Matched_Norm_Sample_Barcode == "BS_FPB0JGKX" ~ "p.Val214Val",
    TRUE ~ HGVSp_Short
  )) %>%
  ...
  dplyr::select(Matched_Norm_Sample_Barcode, Hugo_Symbol,
                ...
                Reference_Allele, HGVSc, HGVSp, HGVSp_Short, Variant_Type)
...
merged_maf <- maftools::read.maf(file.path(results_dir, "germ-somatic-merged-maf.tsv"),
                                 vc_nonSyn = c("Splice_Site", "Silent", "Splice_Region",
                                               "Missense_Mutation", "Nonsense_Mutation",
...
for (id in ids){
  ...
  if (id %in% c("BS_DC7XYYG8", "BS_5WK2KXYW", 
                "BS_DGG7YE8R", "BS_K3PPH522",
                "BS_0S617SMM")){
  
    lollipopPlot(plot_maf, gene = gene, AACol = "HGVSp", 
