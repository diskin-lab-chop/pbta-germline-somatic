...
germline_somatic_cnvs_loh <- germline_somatic_cnvs %>%
  left_join(loh[,c("Kids_First_Biospecimen_ID_normal", "Chromosome", "Loc", "proband_germline_vaf", "proband_tumor_vaf", "loh")], by = c("Kids_First_Biospecimen_ID_normal", "Chromosome", "Loc")) %>%
  distinct()
```
...
loh_gene <- read_tsv(loh_gene_file) %>%
  filter(BS_ID %in% germline$Kids_First_Biospecimen_ID_normal & gene %in% cpgs) %>%
...
```{r}
germline_somatic_cnvs_loh_gene <- germline_somatic_cnvs %>%
  bind_rows(germline_somatic_loh) %>%
  write_tsv(file.path(results_dir, "germline-somatic-cnv-loh.tsv"))
```
