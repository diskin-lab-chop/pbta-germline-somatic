...
loh_file <- file.path("/Users/corbettr/Documents/pbta-germline/LOH/pbta-plp-all-loh.tsv")
...
loh <- loh %>%
  dplyr::mutate(tumor_vaf_recalculated = case_when(
    nchar(alt) > nchar(ref) ~ proband_tumor_alt_depth/proband_tumor_depth,
    TRUE ~ proband_tumor_alt_depth/(proband_tumor_ref_depth + proband_tumor_alt_depth)
  )) %>%
  dplyr::mutate(loh_recalculated = tumor_vaf_recalculated - 0.5)

summary(loh$loh_uncorrected)
summary(loh$loh_recalculated)

sum(loh$loh_uncorrected > 0.3)
sum(loh$loh_uncorrected < -0.3)

sum(loh$loh_recalculated > 0.3)
sum(loh$loh_recalculated < -0.3)
...
germline_somatic_cnvs_loh <- germline_somatic_cnvs %>%
  left_join(loh[,c("Kids_First_Biospecimen_ID_normal", "Kids_First_Biospecimen_ID_tumor", 
                   ...
                   "loh_uncorrected", "tumor_vaf_recalculated", "loh_recalculated")], 
            by = c("Kids_First_Biospecimen_ID_tumor", "Chromosome", "Loc")) %>%
  distinct()

germline_somatic_cnvs_loh[germline_somatic_cnvs_loh$loh_recalculated > 0.3,]

fisher.test(table(germline_somatic_cnvs_loh$loh_uncorrected > 0.3,
      germline_somatic_cnvs_loh$CNV_status_somatic == "loss"))

fisher.test(table(germline_somatic_cnvs_loh$loh_recalculated > 0.2,
      germline_somatic_cnvs_loh$CNV_status_somatic == "loss"))
```
...
germline_somatic_cnvs_loh_gene %>%
  filter(abs(loh_uncorrected) > 0.3) %>%
