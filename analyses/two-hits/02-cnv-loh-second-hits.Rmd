...
loh <- read_tsv(loh_variant_file) %>%
  ...
  dplyr::filter(!is.na(proband_tumor_alt_depth),
                Kids_First_Biospecimen_ID_tumor %in% hist_cohort$Kids_First_Biospecimen_ID_tumor) %>%
  dplyr::mutate(loh_score = proband_tumor_vaf - 0.5)
...
cnv_loh_plot_df %>%
  ggplot(aes(x = CNV_status_somatic, y = loh_score,
  ...
  labs(x = "Germline P/LP variant", y = "variant LOH score", color = "Histology") +
  ylim(c(-0.5,0.8)) +
  theme_Publication()
