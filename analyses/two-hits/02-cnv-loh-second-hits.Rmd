...
germline <- germline %>%
  ...
  dplyr::select(Kids_First_Biospecimen_ID_normal, Kids_First_Participant_ID, 
                ...
                HGVS.c, HGVS.p) %>%
  distinct(Kids_First_Biospecimen_ID_normal, Kids_First_Biospecimen_ID_tumor, Hugo_Symbol, Loc, .keep_all = T)
```
...
cnv_auto <- read_tsv(cnv_auto_file) %>%
  dplyr::filter(biospecimen_id %in% germline$Kids_First_Biospecimen_ID_tumor & gene_symbol %in% cpgs) %>%
...
loh <- read_tsv(loh_variant_file) %>%
  dplyr::filter(start %in% germline$Loc) %>%
...
hist_cohort <- read_tsv(file.path(root_dir, "analyses", "collapse-tumor-histologies", "results", "germline-primary-plus-tumor-histologies-plot-groups-clin-meta.tsv"))
...
loh_gene <- loh_gene %>%
  ...
  dplyr::mutate(is_plp = case_when(
    glue::glue("{Kids_First_Biospecimen_ID_normal}:{Hugo_Symbol}") %in% glue::glue("{germline$Kids_First_Biospecimen_ID_normal}:{germline$Hugo_Symbol}") ~ "Yes",
    TRUE ~ "No"
  )) %>%
  # append plot_group and hex code for plotting
  left_join(germline[,c("Kids_First_Biospecimen_ID_normal", "plot_group")]) %>%
  left_join(plot_mapping[,c("plot_group", "plot_group_hex")]) %>%
  # Create column that has plot_group value for patients with P/LP variants in gene
  dplyr::mutate(plp_group = case_when(
    is_plp == "Yes" | gene_LOH > 0.25 ~ plot_group,
    TRUE ~ NA_character_
  )) %>%
...
loh_gene %>%
  ...
  filter(Hugo_Symbol %in% cpgs_test & Hugo_Symbol != "RAD50") %>%
  
  ggplot(aes(x = is_plp, y = gene_LOH)) + 
  geom_jitter(aes(color = plp_group), width = 0.2, size = 2.5, alpha = 0.85) +
...
loh_gene %>%
  dplyr::filter(group_gene %in% plot_group_gene & group_gene != "NF1:Neurofibroma plexiform") %>%
   ...
   ggplot(aes(x = is_plp, y = gene_LOH)) + 
    geom_jitter(aes(color = plp_group), 
                width = 0.3, size = 2.5, alpha = 0.85) +
    geom_boxplot(alpha = 0.25, outlier.shape = NA) +
    stat_compare_means(method = "wilcox",
                       comparisons = list(c("Yes", "No")),
                       method.args = list(alternative = "greater")) + 
