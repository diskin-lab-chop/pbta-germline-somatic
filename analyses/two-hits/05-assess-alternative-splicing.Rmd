...
splice_psi_df <- splice_psi_df %>%
  ...
  left_join(plp_full[,c("Kids_First_Biospecimen_ID", "Hugo_Symbol", "Loc")]) %>%
  ...
  dplyr::mutate(start_dist = Loc - region_start,
                end_dist = Loc - region_end)
...
```{r}
splice_psi_df <- splice_psi_df %>%
  dplyr::rename(plp_variant_position = Loc) %>%
