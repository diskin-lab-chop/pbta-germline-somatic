...
methyl %>%
  ...
  facet_wrap(~subgroup, nrow = 4,
              labeller = labeller(subgroup = label_wrap_gen(16))) + 
   scale_fill_manual(values = plot_group_palette, 
                      breaks = sort(names(plot_group_palette))) +
  theme_Publication()
...
gene_groups <- plp_summary %>%
  left_join(hist[c("Kids_First_Biospecimen_ID_normal", "subgroup")]) %>%
  group_by(Hugo_Symbol, subgroup) %>%
  summarise(n = n()) %>%
  dplyr::filter(n >= 2) %>%
  dplyr::mutate(gene_group = glue::glue("{Hugo_Symbol}: {subgroup}"))
```
...
```{r}
methyl <- methyl %>%
  left_join(gene_groups[,c("Hugo_Symbol", "subgroup", "gene_group")],
            by = "subgroup") %>%
  ...
  methyl %>%
    ...
    ylim(c(0.35,.7)) +
     scale_fill_manual(values = plot_group_palette, 
                        breaks = sort(names(plot_group_palette))) +
    facet_wrap(~subgroup, nrow = 4,
               labeller = labeller(subgroup = label_wrap_gen(18))) +
    theme_Publication()
