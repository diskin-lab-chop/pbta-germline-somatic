...
methyl_enr_df <- hyper_enr_df %>%
  ...
  dplyr::mutate(p_text = case_when(
    P_Value < 0.01 ~ glue::glue("*p={sprintf('%.1e', P_Value)}"),
...
ggplot(methyl_expr_plot_df,
  ...
  geom_text(data = group_cts,
            aes(x = methyl_group, y = -4, label = group_n),
            size = 3.5,
