...
opc_hist_file <- file.path(input_dir, "histologies.tsv")
...
methylation_specimens <- read_tsv(methylation_specimens_file) %>%
  left_join(opc_hist %>%
              dplyr::select(Kids_First_Biospecimen_ID, match_id)) %>%
...
hist <- hist %>%
  ...
  dplyr::mutate(age_at_diagnosis_years = as.numeric(age_at_diagnosis_days)/365.25) %>%
  dplyr::mutate(age_range = case_when(
    ...
    age_at_diagnosis_years < 20 ~ "[15,20)",
    !is.na(age_at_diagnosis_years) ~ "20+",
    TRUE ~ "Unknown"
  ))
