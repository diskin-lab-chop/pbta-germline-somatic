...
maf <- data.table::fread(file.path(data_dir, "snv-consensus-plus-hotspots.maf.tsv.gz"), 
                         data.table = F) %>%
  filter(Tumor_Sample_Barcode %in% tumor_ids,
         Hugo_Symbol %in% cpgs) %>%
  unique() %>%
  write_tsv(file.path(analysis_dir, "snv-consensus-plus-hotspots-goi.maf.tsv"))
```
